...
(function() {
  ...
  // Update marker: 2025-09-28-22:55-state-fix
  ...
  function shouldExcludeLeaderboardPlayer(name) {
    if (!name) return false;
    const trimmed = name.trim();
    return isLegacyPlayerName(trimmed) || isSimulatedLeaderboardName(trimmed);
  }

  function generatePlayerCode(sessionId) {
    if (!sessionId) return '---';
    const clean = sessionId.replace(/[^a-f0-9]/gi, '');
    const slice = (clean || sessionId).slice(0, 3);
    return slice ? slice.toUpperCase() : '---';
  }

  function formatBotLabel(policyId, fallback = 'Bot') {
    if (!policyId) return fallback;
    const found = state.botPolicies.find(p => p.id === policyId);
    const emoji = emojiMap[policyId] || '🤖';
    const label = found ? found.label : policyId.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
  ...
  async function refreshBotPerformance() {
    ...
    try {
  const response = await fetchJson('/games', { limit: 500 });
      const games = Array.isArray(response?.games) ? response.games : [];

      const aggregate = new Map();
      state.botPolicies.forEach(pol => {
        aggregate.set(pol.id, { wins: 0, games: 0 });
      });

      const cutoff = state.metricsResetDate;
      let recognizedTotal = 0;
      let postCutoffCount = 0;
      let completedCount = 0;
      let missingTimestamps = 0;
      let filteredLegacyCount = 0;
      // Track potential data-quality issues without spamming the UI when games are mid-match.

      games.forEach(game => {
        const policy = game?.policy;
        if (!aggregate.has(policy)) return;

        const playerName = (game?.player_name || '').trim();
        if (isLegacyPlayerName(playerName)) {
          filteredLegacyCount += 1;
          return;
        }

        recognizedTotal += 1;

        let passesCutoff = true;
        if (cutoff) {
          const created = parseIsoTimestamp(game?.created_ts);
          if (!created) {
            missingTimestamps += 1;
            return;
          }
        ...
        const completed = winner === 'bot' || winner === 'user';
        if (!completed) {
          return;
        }
        completedCount += 1;

        const stat = aggregate.get(policy);
        stat.games += 1;
        if (game?.winner === 'bot') {
          stat.wins += 1;
        }
      ...
      const warnings = [];
      if (!state.metricsResetIso) {
        warnings.push('Metrics reset timestamp unavailable; using full history for win rates.');
      } else if (!cutoff) {
        warnings.push(`Metrics reset timestamp "${state.metricsResetIso}" could not be parsed; win rates may be stale.`);
      } else {
        if (recognizedTotal > 0 && postCutoffCount === 0) {
          const cutoffLabel = cutoff.toLocaleString();
          warnings.push(`No /games entries newer than ${cutoffLabel}; win rates are not reflecting recent matches.`);
        }
        if (missingTimestamps > 0) {
          warnings.push(`${missingTimestamps} recent /games entries were missing created_ts and were skipped.`);
        }
      }

      if (recognizedTotal === 0 && filteredLegacyCount > 0) {
        warnings.push('Only simulation games against legacy bots were found; win rates vs human players are unavailable.');
      }
      ...
      state.botPolicies.forEach(pol => {
        ...
        if (!statsEl) return;
        const { wins, games: total } = aggregate.get(pol.id) || { wins: 0, games: 0 };
        const winRate = total ? (wins / total) * 100 : null;
        const rateText = winRate === null ? '—' : `${winRate.toFixed(1)}%`;
        const warningPrefix = hasWarnings ? '⚠️ ' : '';
        statsEl.innerHTML = `<span>${warningPrefix}Win Rate vs Human Players: ${rateText}</span><span>${wins}/${total}</span>`;
        statsEl.classList.toggle('warn', hasWarnings);
      });
    } catch (err) {
      updateDataHealthBanner([`Failed to refresh /games: ${err.message}`]);
  ...
  function bindEvents() {
    ...
    elements.gameOverRefresh?.addEventListener('click', refreshLeaderboard);
